...
const { experienceTypes } = viagensConfig;
...
type ExperienceTypeItem = (typeof experienceTypes.items)[number];

function ExperienceTypeCard({
  item,
  index,
}: {
  item: ExperienceTypeItem;
  index: number;
}): ReactNode {
  return (
    <motion.article
    ...
    >
      <div className="grid min-h-[12.5rem] grid-cols-1 sm:min-h-[13.5rem] sm:grid-cols-[14rem_minmax(0,1fr)] md:min-h-[18rem] md:grid-cols-[18rem_minmax(0,1fr)] lg:min-h-[21rem] lg:grid-cols-[20rem_minmax(0,1fr)]">
        ...
        <div className="relative flex flex-col justify-center px-5 py-6 sm:px-7 sm:py-8 md:px-9 md:py-12 lg:px-10 lg:py-14">
          ...
          <h3 className="relative mb-2 font-display text-xl font-bold tracking-tight text-white md:text-2xl">
            {item.title}
          </h3>
...
export function ViagensExperienceTypes(): ReactNode {
  ...
  return (
    <section
      ref={sectionRef}
      id={experienceTypes.id}
      aria-labelledby="viagens-formatos-heading"
    ...
    >
      ...
      >
        ...
        <div className="relative z-10 mx-auto flex max-w-6xl flex-col gap-10 px-6 md:gap-14 lg:flex-row lg:items-start lg:gap-16">
          ...
          >
            <SectionEyebrow className="mb-3 text-white/72">
              {experienceTypes.eyebrow}
            </SectionEyebrow>
            <div className="mb-5 flex justify-start text-primary md:mb-6">
              <WaveDivider className="h-2.5 w-[min(10rem,36vw)]" />
            </div>
            <SectionTitle
              id="viagens-formatos-heading"
              sans={experienceTypes.titleSans}
              display={experienceTypes.titleDisplay}
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-8 md:gap-16 lg:gap-20 xl:gap-24">
            {experienceTypes.items.map((item, index) => (
              <ExperienceTypeCard key={item.number} item={item} index={index} />
            ))}
          </div>
        </div>
      </div>
